...
export default function URLList() {
  ...
  const { urls, totalPages, loading, setUrls } = useURLs({
    query,
    filter,
    sortBy,
    page,
  ...
  const handleToggle = async (id: string) => {
    ...
    const res = await urlService.update(id, {
      active: !url.active,
      label: url.label,
      url: url.url,
    });
  ...
  return (
    <div className="py-12 min-h-screen max-w-7xl mx-auto px-6 md:px-12">
      {/* Header + Filters */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <Select
            value={filter}
            onValueChange={(v: "all" | "active" | "inactive") => setFilter(v)}
          >
            <SelectTrigger>
          ...
          <Select
            value={sortBy}
            onValueChange={(v: "label" | "created_at") => setSortBy(v)}
          >
            <SelectTrigger>
